...
grouped by config_type. Supports dual model comparison with grouped bars.
...
def plot_mae_comparison(mae_results1: Dict[str, Dict[str, float]], 
                        mae_results2: Dict[str, Dict[str, float]],
                        output_file: str = None, 
                        model1_name: str = "Model 1",
                        model2_name: str = "Model 2",
                        cutoff_multiplier: float = 4):
    """
    Plot MAE results as horizontal grouped bar chart (dual model comparison)
    
    Args:
        mae_results1: Dict[config_type, Dict[property, mae_value]] for model 1
        mae_results2: Dict[config_type, Dict[property, mae_value]] for model 2
        output_file: Output file path
        model1_name: Name of model 1
        model2_name: Name of model 2
        cutoff_multiplier: Cutoff = median * cutoff_multiplier (default: 4)
    """
    ...
    # 根据config数量动态调整y0
    base_height_per_config = 1 * n  # 每个config的基础高度
    y0 = int(max(8*n, n_configs * base_height_per_config))
    
    x0 = 10*n
    dx = int(x0/2.5)
    M = int(3*x0 + 2*dx)  # 3列，2个间隔
    N = y0
    ...
    for idx, (prop, label) in enumerate(zip(properties, property_labels)):
        # 使用GridSpec创建子图 - 3列布局
        col = idx
        ax = fig.add_subplot(gs[0:N, col*(x0+dx):col*(x0+dx)+x0])
        
        # 添加子图标题
        ax.text(-0.15, 1.02, subplot_labels[idx], transform=ax.transAxes,
                fontsize=fontsize+12, fontweight='bold', ha='left', va='top')
        ...
        # Collect MAE values for model 2 (convert to meV)
        mae_values2 = []
        for config_type in config_types:
        ...
        all_values = mae_values1 + mae_values2
            ...
            if val_orig > 0:
                ...
                if clipped:
                    ...
                    ax.text(val_clip - zigzag_width*1.5, y_center, 
                           f'{val_orig:.1f}', 
                           va='center', ha='right', fontsize=fontsize-3, fontweight='bold',
                           bbox=dict(boxstyle='round,pad=0.3', facecolor='yellow', alpha=0.7))
                else:
                    ax.text(val_orig, y_center, 
                           f'{val_orig:.1f}', 
                           va='center', ha='left', fontsize=fontsize-3, fontweight='bold')
        
        # Create bars and labels for model 2
        ...
        ax.set_yticklabels(plot_types, fontsize=fontsize)
        # else:
        #     ax.set_yticklabels([])  # Hide y-axis labels for right two subplots
        
        ax.set_xlabel(f'MAE: {label}', fontsize=fontsize+2, fontweight='bold')
        ...
        ax.legend(loc='center right', fontsize=fontsize+2, framealpha=0.9)
...
Examples:
  ...
  uv run python scripts/plot_mae_by_config.py \
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Forcefield file or directory for model 2 (.txt=NEP, dir=tabGAP)"
    )
    parser.add_argument(
        "-t", "--train-xyz",
        type=str,
    ...
    # Build config_type mapping from model 1
    ...
    # Step 3: Collect data for model 2 (using model 1's config_type mapping)
