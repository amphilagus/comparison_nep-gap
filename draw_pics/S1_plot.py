...
def read_test_xyz(filename, align_for_tabgap=False):
    ...
    while idx < len(lines):
        ...
        if structure:
            # Skip delta and kappa phases
            if structure.get('config_type') in ['bulk_delta_phase', 'bulk_kappa_phase']:
                continue
...
def read_xyz_frames(xyz_file):
    ...
    while i < len(lines):
        ...
        # Skip delta and kappa phases
        properties_line = frame_lines[1]
        config_type_match = re.search(r'Config_type=(\w+)', properties_line)
        if config_type_match:
            config_type = config_type_match.group(1)
            if config_type in ['bulk_delta_phase', 'bulk_kappa_phase']:
                i += n_atoms + 2
                continue
...
def plot_multi_model_comparison(multi_model_data, output_filename, model_names=None):
    ...
    config_colors = ['#1f77b4', '#ff7f0e', '#d62728', '#9467bd', '#8c564b', '#e377c2']
